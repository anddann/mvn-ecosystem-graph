...
public class ArtifactProcessor {
    ...
    private Path downloadFilePlainURL(CustomArtifactInfo info, Path downloadFolder)
        ...
        String classifier = "";
        //handle null values coming from the database, since neo4j does not allow null, we have the string "null"
        if (StringUtils.isNotBlank(info.getClassifier()) && !StringUtils.equals("null", info.getClassifier())) {
